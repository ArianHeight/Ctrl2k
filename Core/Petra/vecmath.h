...
template <typename T>
inline void min_in_place(const vec2_base<T>& op1, const vec2_base<T>& op2, vec2_base<T>& out)
{
	out = { min(op1.x, op2.x), min(op1.y, op2.y) };
}
template <typename T>
inline void min_in_place(const vec3_base<T>& op1, const vec3_base<T>& op2, vec3_base<T>& out)
{
	out = { min(op1.x, op2.x), min(op1.y, op2.y), min(op1.z, op2.z) };
}
template <typename T>
inline void min_in_place(const vec4_base<T>& op1, const vec4_base<T>& op2, vec4_base<T>& out)
{
	out = { min(op1.x, op2.x), min(op1.y, op2.y), min(op1.z, op2.z), min(op1.w, op2.w) };
}
template <typename T>
vec2_base<T> min(const vec2_base<T>& op1, const vec2_base<T>& op2)
{
	return { min(op1.x, op2.x), min(op1.y, op2.y) };
}
template <typename T>
vec3_base<T> min(const vec3_base<T>& op1, const vec3_base<T>& op2)
{
	return { min(op1.x, op2.x), min(op1.y, op2.y), min(op1.z, op2.z) };
}
template <typename T>
vec4_base<T> min(const vec4_base<T>& op1, const vec4_base<T>& op2)
{
	return { min(op1.x, op2.x), min(op1.y, op2.y), min(op1.z, op2.z), min(op1.w, op2.w) };
}
template <typename T>
inline void max_in_place(const vec2_base<T>& op1, const vec2_base<T>& op2, vec2_base<T>& out)
{
	out = { max(op1.x, op2.x), max(op1.y, op2.y) };
}
template <typename T>
inline void max_in_place(const vec3_base<T>& op1, const vec3_base<T>& op2, vec3_base<T>& out)
{
	out = { max(op1.x, op2.x), max(op1.y, op2.y), max(op1.z, op2.z) };
}
template <typename T>
inline void max_in_place(const vec4_base<T>& op1, const vec4_base<T>& op2, vec4_base<T>& out)
{
	out = { max(op1.x, op2.x), max(op1.y, op2.y), max(op1.z, op2.z), max(op1.w, op2.w) };
}
template <typename T>
vec2_base<T> max(const vec2_base<T>& op1, const vec2_base<T>& op2)
{
	return { max(op1.x, op2.x), max(op1.y, op2.y) };
}
template <typename T>
vec3_base<T> max(const vec3_base<T>& op1, const vec3_base<T>& op2)
{
	return { max(op1.x, op2.x), max(op1.y, op2.y), max(op1.z, op2.z) };
}
template <typename T>
vec4_base<T> max(const vec4_base<T>& op1, const vec4_base<T>& op2)
{
	return { max(op1.x, op2.x), max(op1.y, op2.y), max(op1.z, op2.z), max(op1.w, op2.w) };
}
