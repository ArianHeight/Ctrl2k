...
public:
	...
	void addShader(const Shader& shaderobj); //adds shader to render engine
	...
	void drawTextureN(int textureIndex, const glm::vec2& topLeft, const glm::vec2& bottomRight, float depth = 0.0f); //draw texture with normalized coordinates in screen space(0 - 1)
	...
	void drawLine(const glm::vec2& wp, const glm::vec2& wpt, const glm::vec3& colour); //draws a line into world space
	void drawString(int fontIndex, const std::string &string, glm::vec2 cursor, float size = 64.0f, const glm::vec4& colour = glm::vec4(1.0f)); //draws a string onto the screen, left-aligned
	void drawStringInBox(int fontIndex, const std::string& string, glm::vec2 cursor, float width, float size = 64.0f, const glm::vec4& colour = glm::vec4(1.0f)); //draws a string in a box, left-aligned 
